...
class Viewer(object):
    def __init__(self, settings):
        ...
        if isinstance(self.ViewerSettings, cuvis_il.cuvis_viewer_settings_t):
            ...
            if cuvis_il.status_ok != cuvis_il.cuvis_viewer_create(_ptr, self.ViewerSettings):
                raise SDKException()
            self.__handle__ = cuvis_il.p_int_value(_ptr)
        else:
            raise SDKException("Could not open viewer_settings of type {}!".format(type(self.ViewerSettings)))

        pass

    def getDataCount(self, mesu):
        _ptr = cuvis_il.new_p_int()
        if cuvis_il.status_ok != cuvis_il.cuvis_viewer_apply(self.__handle__, mesu.__handle__, _ptr):
            raise SDKException()
        currentView = cuvis_il.p_int_value(_ptr)

        countHandle = cuvis_il.new_p_int()
        if cuvis_il.status_ok != cuvis_il.cuvis_view_get_data_count(currentView, countHandle):
            raise SDKException()
    ...
    def apply(self, mesu):
        ...
        for i in range(dataCount):
            ...
            if cuvis_il.status_ok != cuvis_il.cuvis_view_get_data(currentView, i, view_data):
                raise SDKException()

            if view_data.data.format == CUVIS_imbuffer_format["imbuffer_format_uint8"]:
                return ImageData(img_buf=view_data.data, dformat=view_data.data.format)
            else:
