...
class Session(object):
    def __init__(self, base):
        ...
        if isinstance(base, str) and os.path.exists(base):
            ...
            if cuvis_il.status_ok != cuvis_il.cuvis_session_file_load(base, _ptr):
                raise SDKException()
            self.__handle__ = cuvis_il.p_int_value(_ptr)
        else:
            raise SDKException("Could not open Session File! File not found!")
    ...
    def getFPS(self):
        val = cuvis_il.new_p_int()
        if cuvis_il.status_ok != cuvis_il.cuvis_session_file_get_fps(self.__handle__, val):
            raise SDKException()
        return cuvis_il.p_int_value(val)

    def getOperationMode(self):
        val = cuvis_il.new_p_cuvis_operation_mode_t()
        if cuvis_il.status_ok != cuvis_il.cuvis_session_file_get_operation_mode(self.__handle__, val):
            raise SDKException()
