...
class Worker(object):
    ...
    # Returns a tuple of: (Session in progress: True / False, frames already read, total frames in session)
    def querySessionProgress(self):
        frames_read = cuvis_il.new_p_int()
        frames_total = cuvis_il.new_p_int()

        if self.sessionSet:
            if cuvis_il.status_ok != cuvis_il.cuvis_worker_query_session_progress(self.__handle__, frames_read, frames_total):
                self.sessionSet = False
            else:
    ...
    def setSession(self, base=None, skip_dropped_frames=False):
        ...
        if base is not None:
            if cuvis_il.status_ok != cuvis_il.cuvis_worker_set_session_file(self.__handle__, base.__handle__, 1 if skip_dropped_frames else 0):
                raise SDKException()
            else:
                self.sessionSet = True
        else:
            if cuvis_il.status_ok != cuvis_il.cuvis_worker_set_session_file(self.__handle__, 0, 0):
                raise SDKException()
    ...
    def ingestMesu(self, base):
        ...
        if cuvis_il.status_ok != cuvis_il.cuvis_worker_ingest_mesu(self.__handle__, base.__handle__):
            raise SDKException()
    ...
    def getNextResult(self, timeout=-1):
        ...
        if cuvis_il.status_ok != cuvis_il.cuvis_worker_get_next_result(self.__handle__, this_mesu, this_viewer, timeout):
            raise SDKException()
    ...
    def getQueueLimits(self):
        ...
        if cuvis_il.status_ok != cuvis_il.cuvis_worker_get_queue_limits(self.__handle__, hard, soft):
            raise SDKException()
        return (cuvis_il.p_int_value(hard), cuvis_il.p_int_value(soft)) 

    def setQueueLimits(self, hard, soft):
        if cuvis_il.status_ok != cuvis_il.cuvis_worker_set_queue_limits(self.__handle__, hard, soft):
            raise SDKException()
        pass
    ...
    def setWorkerCanDrop(self, val):
        if cuvis_il.status_ok != cuvis_il.cuvis_worker_set_drop_bahvior(self.__handle__, 1 if val else 0):
            raise SDKException()
        
    def getWorkerCanDrop(self):
        val = cuvis_il.new_p_int()
        if cuvis_il.status_ok != cuvis_il.cuvis_worker_get_drop_behavior(self.__handle__, val):
            raise SDKException()
