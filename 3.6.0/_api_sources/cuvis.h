...
enum cuvis_reference_type_t
{
  ...
  /** an averaged white spectrum of some sort, can be used as a replacement for the white reference.
    uses the spatial aspect of white correction in the reflectance calculation */
  Reference_WhiteSpectrum = 5,
  /** an spectrum curve describing the white target in terms of reflectance values that got measured,
    if not supplied a 100 percent white target will be assumed */
  Reference_TargetSpectrum = 6
};
...
/** @brief An averaged spectrum of raw sensor counts.
   *
   * The spectrum analogue of \ref cuvis_imbuffer_t: one struct carries the samples and
   * the acquisition context (bit depth, integration time) that makes counts
   * interpretable, and the setter and the getter both speak it. Used as the white
   * reference for reflectance calculation in place of a full white measurement, see
   * \ref cuvis_proc_cont_set_reference_white_spectrum.
   *
   * Filled by a getter, the pointers reference SDK memory and are valid as long as the
   * processing context is valid and the reference slot unchanged, i.e. until the
   * reference is set again, cleared, or the context freed - the same contract as
   * \ref cuvis_imbuffer_t. Copy the arrays out to keep them longer.
   */
struct cuvis_sensor_spectrum_t
{
  /** wavelengths in nanometers, array of length \ref count */
  float const* wavelengths;

  /** raw sensor counts, array of length \ref count */
  uint16_t const* values;

  /** number of samples */
  uint32_t count;

  /** @brief how many bits of each count are significant, 1 to 16
      *
      * 4095 off a 12 bit sensor is full scale; off a 16 bit one it is a sixteenth of
      * it, so counts are meaningless without the depth. A count above the full scale
      * this depth implies is rejected.
      */
  uint16_t effective_bit_depth;

  /** integration time in milliseconds the counts were recorded with */
  double integration_time;
};

#define CUVIS_SENSOR_SPECTRUM struct cuvis_sensor_spectrum_t

/** @brief The reflectance spectrum of the white target used in the scene.
   *
   * Values are fractions: 0.0 for 0% and 1.0 for 100% reflectivity. A dimensionless
   * curve, so it carries no acquisition context. Pointer validity as in
   * \ref cuvis_sensor_spectrum_t. Used as the target reference, see
   * \ref cuvis_proc_cont_set_reference_target_spectrum.
   */
struct cuvis_target_spectrum_t
{
  /** wavelengths in nanometers, array of length \ref count */
  float const* wavelengths;

  /** reflectance values as fractions (1.0 for 100%), array of length \ref count */
  float const* values;
...
#define CUVIS_TARGET_SPECTRUM struct cuvis_target_spectrum_t
...
*/
SDK_CAPI CUVIS_STATUS SDK_CCALL cuvis_measurement_clear_cube(CUVIS_MESU i_mesu);
...
*/
SDK_CAPI CUVIS_STATUS SDK_CCALL cuvis_measurement_clear_implicit_reference(CUVIS_MESU i_mesu, CUVIS_REFERENCE_TYPE i_type);
...
/** @brief Set the averaged white reference spectrum from raw sensor counts.
    *
    * Replaces a white reference measurement for reflectance calculation; the two white
    * sources are mutually exclusive, setting one clears the other. The spectrum is
    * copied, the caller keeps ownership of the arrays.
    *
    * @param[in] i_procCont The handle of the processing context
    * @param[in] i_spectrum The spectrum to set into the @ref Reference_WhiteSpectrum
    * slot; see @ref cuvis_sensor_spectrum_t for the field semantics
    * @return @ref status_ok if the spectrum was set.
    */
SDK_CAPI CUVIS_STATUS SDK_CCALL cuvis_proc_cont_set_reference_white_spectrum(CUVIS_PROC_CONT i_procCont, CUVIS_SENSOR_SPECTRUM const* i_spectrum);

/** @brief Borrow the white reference spectrum.
    *
    * @param[in] i_procCont The handle of the processing context
    * @param[out] o_pSpectrum Filled with pointers into SDK memory, count, effective bit
    * depth and integration time; see @ref cuvis_sensor_spectrum_t for their validity
    * @return @ref status_ok if the spectrum was read, @ref status_not_available when
    * the @ref Reference_WhiteSpectrum slot is empty.
    */
SDK_CAPI CUVIS_STATUS SDK_CCALL cuvis_proc_cont_get_reference_white_spectrum(CUVIS_PROC_CONT i_procCont, CUVIS_SENSOR_SPECTRUM* o_pSpectrum);

/** @brief Set the white-target reflectance spectrum.
    *
    * Overrides the default target of 100% reflectivity across the whole range. The
    * spectrum is copied, the caller keeps ownership of the arrays.
    *
    * @param[in] i_procCont The handle of the processing context
    * @param[in] i_spectrum The spectrum to set into the @ref Reference_TargetSpectrum
    * slot; see @ref cuvis_target_spectrum_t for the value convention
    * @return @ref status_ok if the spectrum was set.
    */
SDK_CAPI CUVIS_STATUS SDK_CCALL cuvis_proc_cont_set_reference_target_spectrum(CUVIS_PROC_CONT i_procCont, CUVIS_TARGET_SPECTRUM const* i_spectrum);

/** @brief Borrow the target reflectance spectrum.
    *
    * @param[in] i_procCont The handle of the processing context
    * @param[out] o_pSpectrum Filled with pointers into SDK memory and the count; see
    * @ref cuvis_target_spectrum_t for their validity and the value convention
    * @return @ref status_ok if the spectrum was read, @ref status_not_available when
    * the @ref Reference_TargetSpectrum slot is empty.
    */
SDK_CAPI CUVIS_STATUS SDK_CCALL cuvis_proc_cont_get_reference_target_spectrum(CUVIS_PROC_CONT i_procCont, CUVIS_TARGET_SPECTRUM* o_pSpectrum);
...
/** @brief Creates an additional exporter handle 
    *
    * Creates an additional handle that points to the same instance as the supplied handle
    * 
    * @param[in] i_exporter The handle of the exporter to copy
    * @param[out] o_pExporter The new handle of the exporter.
    * @returns @ref status_ok if the exporter handle could be doubled
    */
SDK_CAPI CUVIS_STATUS SDK_CCALL cuvis_exporter_copy_handle(CUVIS_EXPORTER i_exporter, CUVIS_EXPORTER* o_pExporter);
...
/** @brief Creates an additional worker handle 
    *
    * Creates an additional handle that points to the same instance as the supplied handle
    * 
    * @param[in] i_worker The handle of the worker to copy
    * @param[out] o_pWorker The new handle of the worker.
    * @returns @ref status_ok if the worker handle could be doubled
    */
SDK_CAPI CUVIS_STATUS SDK_CCALL cuvis_worker_copy_handle(CUVIS_WORKER i_worker, CUVIS_WORKER* o_pWorker);
...
ALLOCATE_AND_FREE(CUVIS_SENSOR_SPECTRUM, sensor_spectrum);
ALLOCATE_AND_FREE(CUVIS_TARGET_SPECTRUM, target_spectrum);
...
/**
\addtogroup cuvis_cuda Cuda Device Memory
 @{
*/

enum
{
  CUVIS_CUDA_IPC_BACKEND_AUTO = 0,   /* do not care about selected ipc backend just use one */
  CUVIS_CUDA_IPC_BACKEND_POOL = 1,   /* IPC-capable memory pool (zero-copy fast path; needs an exportable pool) */
  CUVIS_CUDA_IPC_BACKEND_LEGACY = 2, /* dedicated cudaMalloc + cudaIpcGetMemHandle (copy-on-export) */
  CUVIS_CUDA_IPC_BACKEND_VMM = 3     /* driver-API cuMem* + cuMemExportToShareableHandle (copy-on-export) */
};

enum
{
  CUVIS_CUDA_IPC_HANDLE_NONE = 0,      /* legacy: not applicable */
  CUVIS_CUDA_IPC_HANDLE_WIN32 = 1,     /* VMM: NT handle (needs DuplicateHandle) */
  CUVIS_CUDA_IPC_HANDLE_WIN32_KMT = 2, /* VMM: KMT handle (globally valid, copyable) */
  CUVIS_CUDA_IPC_HANDLE_POSIX_FD = 3   /* VMM: POSIX file descriptor */
};

/* 64 == sizeof(cudaIpcMemHandle_t); also fits an 8-byte OS handle / 4-byte fd. */
enum
{
...
struct cuvis_cuda_ipc_descriptor_t
{
  int32_t backend; /* CUVIS_CUDA_IPC_BACKEND_* */
  int32_t device_ordinal;
  int32_t handle_type; /* CUVIS_CUDA_IPC_HANDLE_* */
  uint32_t blob_len;
  uint64_t size;
  uint64_t alloc_size;
  uint64_t offset;
  uint64_t exporter_pid;
  uint8_t blob[CUVIS_CUDA_IPC_BLOB_MAX]; /* pool OS shareable handle */
  uint32_t ptr_blob_len;
  uint32_t _pad;                                 /* keep ptr_blob 8-byte aligned / size a round 184 */
  uint8_t ptr_blob[CUVIS_CUDA_IPC_PTR_BLOB_MAX]; /* cudaMemPoolPtrExportData */
};
...
/** Raw device pointer + size + device for wrapping in-process (e.g. as a torch tensor
 *  via __cuda_array_interface__ / DLPack). Does NOT create an IPC handle. The device_ptr is
 *  read-only from the SDK's side; external code that maps it may still write the memory. */
struct cuvis_cuda_mem_view_t
{
...
/** @brief return the raw device pointer + size + device index for wrapping it in the same process.
  * @param[in] i_mem The cuda memory handle
  * @param[out] o_pView Returns device pointer + size + device index
  */
SDK_CAPI CUVIS_STATUS SDK_CCALL cuvis_cuda_mem_get_view(CUVIS_CUDA_MEM i_mem, CUVIS_CUDA_MEM_VIEW* o_pView);

/** @brief Make a copy of the supplied cuda memory handle. Afterwards both handles need to be freed for the underlying memory to be released.
  * @param[in] i_mem The cuda memory handle
  * @param[out] o_pCopy The new cuda memory handle
  */
SDK_CAPI CUVIS_STATUS SDK_CCALL cuvis_cuda_mem_copy_handle(CUVIS_CUDA_MEM i_mem, CUVIS_CUDA_MEM* o_pCopy);

/* Image buffer described by a CUDA device handle. Mirrors CUVIS_IMBUFFER field-for-field,
 * except the host `raw` pointer is replaced by a `handle` to the device buffer (open it
 * same-process via cuvis_cuda_mem_get_view + DLPack, or share it cross-process by calling
 * cuvis_cuda_ipc_handle_create + cuvis_cuda_ipc_get_descriptor). Free `handle` with
 * cuvis_cuda_mem_free. */
struct cuvis_cuda_imbuffer_t
{
  CUVIS_CUDA_MEM handle; /* shareable device buffer */
  uint32_t bytes;        /* bytes per element */
  uint32_t length;       /* total bytes */
  uint32_t width;
  uint32_t height;
  uint16_t channels;
  CUVIS_IMBUFFER_FORMAT format;
  const uint32_t* wavelength; /* length == channels for cubes, else nullptr */
};
#define CUVIS_CUDA_IMBUFFER struct cuvis_cuda_imbuffer_t

SDK_CAPI CUVIS_STATUS cuvis_measurement_get_data_image_cuda(CUVIS_MESU i_mesu, const CUVIS_CHAR* i_key, CUVIS_CUDA_IMBUFFER* o_pBuf);

/* ---- cross-process IPC export ---- */

/** @brief Registers an cuda memory allocation to be used with cudas IPC protocols. 
  * 
  * This is only supported for specific implementations. For the memory to be released both handles need to be freed.
  * @param[in] i_mem The cuda memory handle
  * @param[out] o_pIpc The new cuda ipc handle
  */
SDK_CAPI CUVIS_STATUS SDK_CCALL cuvis_cuda_ipc_handle_create(CUVIS_CUDA_MEM i_mem, int i_backend, CUVIS_CUDA_IPC* o_pIpc);

/** @brief Returns the IPC specific information of the underlying handle. 
  * 
  * The meaning of the values differs depending on which cuda backend IPC protocol is used.
  * @param[in] i_mem The cuda memory handle
  * @param[out] o_pDesc The IPC descriptor that is going to be filled
  */
SDK_CAPI CUVIS_STATUS SDK_CCALL cuvis_cuda_ipc_get_descriptor(CUVIS_CUDA_IPC i_ipc, CUVIS_CUDA_IPC_DESCRIPTOR* o_pDesc);

/** @brief Frees a cuda ipc handle
  * @param[in] i_ipc The cuda memory handle
  */
SDK_CAPI CUVIS_STATUS SDK_CCALL cuvis_cuda_ipc_handle_free(CUVIS_CUDA_IPC* i_ipc);

/** @brief Frees a cuda memory hanlde
  * @param[in] i_mem The cuda memory handle
  */
SDK_CAPI CUVIS_STATUS SDK_CCALL cuvis_cuda_mem_free(CUVIS_CUDA_MEM* i_mem);

/** @brief Check if a specific IPC backend is available. 
  * Support of the different IPC backends varies
  * @param[in] i_backend The backend to be checked. See CUVIS_CUDA_IPC_BACKEND_ for supported values
  * @param[out] o_pAvailable Indicates if the request IPC backend is available
  */
SDK_CAPI CUVIS_STATUS SDK_CCALL cuvis_cuda_ipc_backend_available(int i_backend, int* o_pAvailable);
