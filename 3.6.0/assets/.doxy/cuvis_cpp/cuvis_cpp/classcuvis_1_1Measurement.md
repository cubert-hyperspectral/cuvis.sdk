...
|  [**CudaImage**](classcuvis_1_1CudaImage.md) | [**cuda\_image**](#function-cuda_image) (std::string [**const**](structcuvis_1_1image__t.md) & key) const<br>_Get image data from the measurement without moving it off the CUDA device._  |
...
### function cuda\_image 

_Get image data from the measurement without moving it off the CUDA device._ 
```C++
CudaImage cuvis::Measurement::cuda_image (
...
Deliberately not part of the eagerly populated [**get\_imdata**](classcuvis_1_1Measurement.md#function-get_imdata) map: acquiring a device buffer is an explicit, per-key request. The host and device copies coexist, so calling this after the host fetch is fine.


Requires the SDK to be running in GPU mode (force\_gpu\_mode="cuda" in cuvis.settings); otherwise the cube is not device-backed and this throws. Throws [**cuda\_unavailable\_error**](classcuvis_1_1cuda__unavailable__error.md) when the loaded library has no CUDA API at all. 
